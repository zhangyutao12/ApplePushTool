...
- (void)postWithPayload:(NSString *)payload
  ...
  if (topic) {
      if ([payloadType isEqualToString:@"liveactivity"]) {
          if (![topic hasSuffix:@".push-type.liveactivity"]) {
              topic = [topic stringByAppendingFormat:@".push-type.liveactivity"];
          }
      }
  ...
  NSURLSessionDataTask *task = [self.session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (r == nil ||  error) {
            ...
            NSError *perror;
            NSDictionary *dict = data.length > 0 ? [NSJSONSerialization JSONObjectWithData:data options:0 error:&perror] : @{};
