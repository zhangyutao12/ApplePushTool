...
@property (nonatomic, strong) LiveActivityWindowController *liveActivityWindowController;
@property (nonatomic, strong) GenericPushWindowController *normalPushWindowController;
@property (nonatomic, strong) GenericPushWindowController *voipPushWindowController;
@end
...
- (void)setupPushTypeSelector {
    NSArray *subviews = [self.view.subviews copy];
    for (NSView *subview in subviews) {
        [subview removeFromSuperview];
    }

    NSTextField *titleLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 300, self.view.bounds.size.width, 34)];
    titleLabel.stringValue = @"SmartPush";
    titleLabel.alignment = NSTextAlignmentCenter;
    titleLabel.font = [NSFont systemFontOfSize:28 weight:NSFontWeightSemibold];
    titleLabel.bezeled = NO;
    titleLabel.drawsBackground = NO;
    titleLabel.editable = NO;
    titleLabel.selectable = NO;
    titleLabel.autoresizingMask = NSViewWidthSizable | NSViewMinYMargin | NSViewMaxYMargin;
    [self.view addSubview:titleLabel];

    NSTextField *subtitleLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 266, self.view.bounds.size.width, 22)];
    subtitleLabel.stringValue = @"选择要调试的 APNs 推送类型";
    ...
    subtitleLabel.autoresizingMask = NSViewWidthSizable | NSViewMinYMargin | NSViewMaxYMargin;
    [self.view addSubview:subtitleLabel];

    CGFloat buttonWidth = 180.0;
    CGFloat buttonHeight = 44.0;
    CGFloat gap = 18.0;
    CGFloat startX = (self.view.bounds.size.width - buttonWidth * 3.0 - gap * 2.0) / 2.0;
    CGFloat y = 196.0;

    NSButton *normalButton = [self selectorButtonWithTitle:@"普通推送" frame:NSMakeRect(startX, y, buttonWidth, buttonHeight) action:@selector(openNormalPushWindow:)];
    NSButton *voipButton = [self selectorButtonWithTitle:@"VoIP 推送" frame:NSMakeRect(startX + buttonWidth + gap, y, buttonWidth, buttonHeight) action:@selector(openVoipPushWindow:)];
    NSButton *liveActivityButton = [self selectorButtonWithTitle:@"实时活动" frame:NSMakeRect(startX + (buttonWidth + gap) * 2.0, y, buttonWidth, buttonHeight) action:@selector(openLiveActivityWindow:)];

    [self.view addSubview:normalButton];
    [self.view addSubview:voipButton];
    [self.view addSubview:liveActivityButton];
}

- (NSButton *)selectorButtonWithTitle:(NSString *)title frame:(NSRect)frame action:(SEL)action {
...
- (void)openNormalPushWindow:(id)sender {
    if (!self.normalPushWindowController) {
        self.normalPushWindowController = [[GenericPushWindowController alloc] initWithTitle:@"普通推送"
                                                                                    pushType:@"alert"
                                                                                 topicSuffix:nil
                                                                             defaultPriority:10];
    }
    [self showWindowController:self.normalPushWindowController];
}

- (void)openVoipPushWindow:(id)sender {
    if (!self.voipPushWindowController) {
        self.voipPushWindowController = [[GenericPushWindowController alloc] initWithTitle:@"VoIP 推送"
                                                                                  pushType:@"voip"
                                                                               topicSuffix:@".voip"
                                                                           defaultPriority:10];
    }
    [self showWindowController:self.voipPushWindowController];
}

- (void)openLiveActivityWindow:(id)sender {
    if (!self.liveActivityWindowController) {
        self.liveActivityWindowController = [[LiveActivityWindowController alloc] init];
    }
    [self showWindowController:self.liveActivityWindowController];
}
...
- (IBAction)push:(id)sender {
    ...
    NSString *collapseID = [self.collapseIDTextView.stringValue stringByReplacingOccurrencesOfString:@" " withString:@""];
